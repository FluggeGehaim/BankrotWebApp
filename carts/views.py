...
def cart_add(request):
    ...
    if request.user.is_authenticated:
        carts = Cart.objects.filter(user=request.user, lot=lot)
        ...
        if carts.exists():
            ...
            Cart.objects.create(user=request.user, lot=lot, quantity=1)
            
    else:
        carts = Cart.objects.filter(session_key=request.session.session_key, lot=lot)
        ...
        if carts.exists(): 
            ...
            Cart.objects.create(
                session_key=request.session.session_key, lot=lot, quantity=1
            )
